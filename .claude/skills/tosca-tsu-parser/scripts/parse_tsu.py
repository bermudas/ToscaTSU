...
# Walk the test case
# ──────────────────────────────────────────────────────────────────────────────
tc      = next(x for x in data['Entities'] if x['ObjectClass'] == 'TestCase')
project = next(x for x in data['Entities'] if x['ObjectClass'] == 'TCProject')
...
env_vars_used = {}   # env_var_name → list of (raw_name, hint) tuples for trace
...
if gen_json:
    ...
    json_path = out_dir / (tsu_path.stem + '_steps.json')
    json_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding='utf-8')
    print(f"[✓] Steps JSON   → {json_path.name}  ({len(unmapped_actions)} unmapped action(s))")
...
HTML = ('<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">'
        ...
        '}'
        '</script></body></html>')
report_path = out_dir / (tsu_path.stem + '_report.html')
...
pw_dir = out_dir / 'playwright-test'
...
# .env.example — list every env var the generated spec references, so users
# know exactly what to provide. Source: env_vars_used populated by safe_env().
...
if env_vars_used:
    env_lines.append("# Credential / config parameters referenced by spec actions:")
    for name in sorted(env_vars_used):
        sources = env_vars_used[name]
        # Show first source as a hint
        hint = f" (from CP[{sources[0][0]}]" + (f" / {sources[0][1]}" if sources[0][1] else "") + ")"
        env_lines.append(f"{name}={hint}")
    env_lines.append("")
...
if not spec_path.exists() or force:
    ...
    print(f"[✓] Test spec    → {spec_path.name}  ({len(unmapped_actions)} TODO unmapped)")
else:
